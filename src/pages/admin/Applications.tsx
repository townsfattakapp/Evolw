...
export function AdminApplications() {
  ...
  const updateAppStatus = async (app: any, newStatus: string) => {
    try {
  ...
  const exportToCSV = () => {
    ...
    const headers = ["Date", "Name", "Email", "Phone", "Job Title", "LinkedIn", "Portfolio", "Status", "Message"];
    ...
    link.href = url;
    link.setAttribute("download", `evolw-job-applications-${new Date().toISOString().split('T')[0]}.csv`);
          ...
          <div className="relative">
            ...
            {isFilterOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-evolw-black border border-evolw-gray-200 dark:border-white/10 rounded-xl shadow-lg z-10 overflow-hidden">
                <div className="py-1">
                  {['All', 'New', 'Reviewed', 'Rejected', 'Hired'].map(status => (
                    <button
                      key={status}
                      onClick={() => {
        ...
        <div className="overflow-x-auto min-h-[400px]">
          {filteredApps.length === 0 ? (
            <div className="p-12 text-center text-evolw-gray-500">
              {apps.length === 0 ? "No job applications have been submitted yet." : "No applications match your search and filter criteria."}
            </div>
          ) : (
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white dark:bg-evolw-slate text-sm uppercase tracking-wider text-evolw-gray-500 border-b border-evolw-gray-200 dark:border-white/5">
                  ...
                  <th className="px-8 py-5 font-semibold">Links</th>
              ...
              <tbody className="divide-y divide-evolw-gray-100 dark:divide-white/5">
                {filteredApps.map((app) => (
                  <tr key={app.id} className="hover:bg-evolw-gray-50/80 dark:hover:bg-white/5 transition-colors group">
                    <td className="px-8 py-6">
                      <p className="font-bold text-base">{app.name}</p>
                      <p className="text-sm text-evolw-gray-500">{app.email}</p>
                      <p className="text-xs text-evolw-gray-400 font-mono mt-1">{app.phone || 'No phone'}</p>
                      <p className="text-xs text-evolw-gray-400 mt-1">{app.date}</p>
                    </td>
                    <td className="px-8 py-6">
                    ...
                    <td className="px-8 py-6 space-y-2">
                      {app.resumeUrl && (
                        <a href={app.resumeUrl} target="_blank" rel="noopener noreferrer" className="flex items-center text-sm font-medium text-evolw-accent hover:underline bg-evolw-accent/10 px-2 py-1 rounded-md w-fit mb-2">
                          <ExternalLink className="w-3.5 h-3.5 mr-1.5" />
                          View Resume
                        </a>
                      )}
                      {app.linkedin && app.linkedin !== "Not provided" && (
                        <a href={app.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center text-sm font-medium text-blue-600 hover:underline">
                          <ExternalLink className="w-3.5 h-3.5 mr-1.5" />
                          LinkedIn
                        </a>
                      )}
                      {app.portfolio && (
                        <a href={app.portfolio} target="_blank" rel="noopener noreferrer" className="flex items-center text-sm font-medium text-evolw-gray-600 dark:text-gray-400 hover:underline">
                          <ExternalLink className="w-3.5 h-3.5 mr-1.5" />
                          Portfolio
                        </a>
                      )}
                    ...
                    <td className="px-8 py-6 max-w-xs">
                      ...
                      >
                        <p className={`text-sm text-evolw-gray-600 dark:text-evolw-gray-400 ${expandedMsgId === app.id ? 'whitespace-normal' : 'truncate'} transition-all`}>
                          {app.message || "No cover letter provided."}
                        </p>
                        {expandedMsgId !== app.id && app.message?.length > 40 && (
                    ...
                    <td className="px-8 py-6">
                      <select
                        ...
                        className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider appearance-none cursor-pointer outline-none shadow-sm ${
                          app.status === 'New' ? 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400' :
                          app.status === 'Reviewed' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400' :
                          app.status === 'Hired' ? 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400' :
